...
/**
 * Scheduled background maintenance jobs (auto-backup, ungraded reminders, weekly digest).
 * Every job first reads the relevant `system` settings toggle and no-ops if disabled, so a
 * fresh DB (defaults OFF/sensible) stays quiet. All DB/email work is wrapped in try/catch and
 * logged — a cron throwing must never crash the app.
 */
@Injectable()
export class MaintenanceService {
  private readonly logger = new Logger(MaintenanceService.name);

  // Keep at most this many backup files on disk; older ones are pruned best-effort.
  ...
  /** Read the `system` settings doc (or null if missing/unreadable). */
  ...
  // ────────────────────────────────────────────────────────────────────────
  // 1) Auto-backup — daily at 02:00. Honors data.autoBackup + backupFrequency.
  // ────────────────────────────────────────────────────────────────────────
  ...
  async autoBackup(): Promise<void> {
    try {
      ...
      const now = new Date();
      if (frequency === 'weekly' && now.getDay() !== 1) return; // Monday only
      if (frequency === 'monthly' && now.getDate() !== 1) return; // 1st of month only
      // 'daily' (or anything else) → run every day.
  ...
  /** Delete all but the latest MAX_BACKUPS backup files (best-effort). */
  ...
  // ────────────────────────────────────────────────────────────────────────
  // 2) Ungraded reminder — daily at 08:00. Honors notifications.remindUngraded.
  // ────────────────────────────────────────────────────────────────────────
  ...
  async remindUngraded(): Promise<void> {
    try {
      ...
      // Submission → Attempt → Exercise → owner (Exercise.userId).
      ...
      // Count pending submissions per teacher (owner).
  ...
  // ────────────────────────────────────────────────────────────────────────
  // 3) Weekly digest — once a week. Honors notifications.weeklyDigest.
  // ────────────────────────────────────────────────────────────────────────
