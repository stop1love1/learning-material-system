...
const TWEAKS_STYLE = `
  .twk-fab{position:fixed;right:16px;bottom:16px;z-index:2147483645;width:46px;height:46px;border-radius:50%;
    border:.5px solid rgba(0,0,0,.12);background:rgba(250,249,247,.92);color:#29261b;cursor:pointer;
    display:flex;align-items:center;justify-content:center;
    -webkit-backdrop-filter:blur(24px) saturate(160%);backdrop-filter:blur(24px) saturate(160%);
    box-shadow:0 4px 16px rgba(0,0,0,.18)}
  .twk-fab:hover{background:#fff}
  ...
  .twk-swatch{appearance:none;-webkit-appearance:none;width:56px;height:22px;
    border:.5px solid rgba(0,0,0,.1);border-radius:6px;padding:0;cursor:pointer;background:transparent;flex-shrink:0}
  ...
  if (!open) {
    return (
      <>
        ...
        <button className="twk-fab" aria-label="Mở Tweaks" title="Tuỳ chỉnh giao diện" onClick={() => setOpen(true)}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
            <path d="M4 7h9M17 7h3M4 12h4M12 12h8M4 17h11M19 17h1M13 5v4M8 10v4M17 15v4" />
          </svg>
        </button>
      </>
    );
...
export function TweakRow({ label, value, children, inline = false }: { label: ReactNode; value?: ReactNode; children?: ReactNode; inline?: boolean }) {
  return (
...
export function TweakToggle({ label, value, onChange }: { label: ReactNode; value: boolean; onChange: (v: boolean) => void }) {
  return (
...
export function TweakRadio({ label, value, options, onChange }: any) {
  ...
  if (!fitsAsSegments) {
    ...
    return <TweakSelect label={label} value={value} options={options} onChange={(s: string) => onChange(resolve(s))} />;
  }
...
export function TweakButton({ label, onClick, secondary = false }: { label: ReactNode; onClick?: () => void; secondary?: boolean }) {
  return (
