...
- **"Kho tài liệu" files are external-link-first**: `FileItem.source` defaults to `external` and stores a `url` (a `pre('validate')` hook requires `url` for external / `fileKey` for internal). No file storage is operated.
